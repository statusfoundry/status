...
- `PluginRuntimeService` now provides the first app-facing execution path for installed declarative plugins. The macOS and iOS integrations screens expose a Run action for the installed Website Uptime plugin, and the settings screens keep the same registry check as a diagnostic shortcut. The run path executes `requests.json` and `mappings.json`, stores the normalized resource/event output locally, records the job, and writes audit output. This is intentionally a narrow proving path; generic per-plugin setup forms, account editing, schedule controls, credential injection, and background runners are still planned work.
