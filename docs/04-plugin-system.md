...
`setup.schema.json` describes non-secret account setup fields that the app renders with native, app-owned controls. The installed plugin projection exposes this schema to the Integrations UI, so labels, placeholders, and required state come from the plugin package while validation, persistence, and execution stay in StatusCore.

Current implementation supports the first single-field path used by Website Uptime. Multi-field setup, secret fields, credential injection, and multi-account editing remain planned work.
...
- `PluginRuntimeService` now provides the first app-facing execution path for installed declarative plugins. The macOS and iOS integrations screens expose a setup field and Run action for the installed Website Uptime plugin, and the settings screens keep the registry check as a diagnostic shortcut. Installed plugin setup metadata is projected from the local package into the app-owned setup row, so the Website label and placeholder come from `setup.schema.json`. The setup path saves the user-configured host as non-secret account configuration; the run path enqueues manual or due cron trigger jobs, executes `requests.json` and `mappings.json`, stores the normalized resource/event output locally, evaluates inserted events against stored rules, dispatches safe local effects through the platform shell, records the job/action runs, and writes audit output. This is intentionally a narrow proving path; multi-field setup rendering, multi-account editing, user-facing schedule controls, credential injection, and OS-level background runners are still planned work.
