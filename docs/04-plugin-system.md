...
Each plugin must declare its own app-owned visual identity through `icon` and `accentColor`. `icon` is an SF Symbol name, optionally prefixed with `sf:`, and `accentColor` is a `#RRGGBB` hex color. These fields are metadata only: plugins still do not ship custom UI, and Status decides how the icon/color appear in the app sidebar, collapsed app strip, plugin catalog, app settings window, notifications, and future mobile surfaces.

Official plugins must also include `icon.svg`. The manifest `icon` remains the native fallback symbol; `icon.svg` is the preferred asset for all rendering surfaces. The package builder includes `icon.svg` and `README.md` in the signed archive, the loader projects the icon as package metadata, and the shared app-owned icon renderer uses it across the plugin catalog, app sidebar, collapsed app strip, dashboard tiles, and app settings.

`icon.svg` must be a **flat single-color shape** using `currentColor` for all fill or stroke values. It must not contain any background, chrome, rounded container, gradient, or multi-color paths. Status owns the tile background, sizing, and tinting; the plugin only provides the silhouette. The SVG must be UTF-8, root `<svg>`, `24×24` viewBox, 32 KiB or smaller, and deterministic under packaging. Status rejects executable or remotely loaded SVG content: scripts, event-handler attributes, `foreignObject`, embedded HTML, remote images, `style` elements, inline `style` attributes, external `href`/`xlink:href` values, and any remote or executable URL scheme. Validation also enforces element, attribute, reference, and path-data limits so oversized or excessively complex provider marks fail closed. Official bundled plugins fail validation if the asset is missing or contains a background or hardcoded color. Local-dev and third-party packages may omit it and rely on the manifest SF Symbol fallback.
...
OAuth plugins declare public provider metadata only: `provider`, `applicationId`, and an `oauth2` block. Callback-based providers use `grantType: "authorization-code"` or omit `grantType`, then declare `authorizationUrl`, `tokenUrl`, `redirectUri`, optional `scopes`, and optional extra authorization parameters. Providers that are native-friendly through device authorization use `grantType: "device-code"`, `deviceAuthorizationUrl`, `tokenUrl`, and optional `scopes`.

The manifest must request `oauth`, `keychain`, and `network`. Callback `redirectUri` values must use `com.statusfoundry.status.oauth:/{provider-slug}` where `{provider-slug}` exactly matches `auth.provider`. Hosts used by `authorizationUrl`, `deviceAuthorizationUrl`, and `tokenUrl` must be listed in `domains` because Status performs those network calls on the plugin's behalf. The native setup flow must not start the OAuth connection until the user has granted those permissions. Status owns authorization-code + PKCE flow, device-code flow, callback `state` validation, token storage, refresh behavior, request header injection, and audit output. Plugins never receive access tokens or refresh tokens directly.
