...
Current implementation renders and stores plain setup fields (`text`, `url`, `hostname`, `number`, `toggle`, and `select`) as non-secret local account configuration. Bearer-token auth fields render as masked native inputs and store the token in Keychain; SQLite stores only the `kc_` credential reference on the account row. Multi-account editing, additional auth types, and user-facing schedule controls remain planned work.
...
- `PluginRuntimeService` now provides the first app-facing execution path for installed declarative plugins. The macOS and iOS integrations screens render setup and bearer-token auth fields from installed plugin package metadata and expose a Run action for any installed plugin with a configured account and enabled manual trigger; the settings screens keep the Website registry check as a diagnostic shortcut. Installed plugin setup/auth metadata is projected from the local package into the app-owned setup row, so labels, placeholders, options, defaults, and required state come from `setup.schema.json` and `auth.json`. The setup path saves non-secret user configuration locally and bearer tokens in Keychain; the run path enqueues manual or due cron trigger jobs, injects bearer credentials at request time, executes `requests.json` and `mappings.json`, stores normalized resources/events/metric points locally, evaluates inserted events against stored rules, dispatches safe local effects through the platform shell, records the job/action runs, and writes audit output. This is intentionally a narrow proving path; multi-account editing, additional auth types, user-facing schedule controls, metric baseline events, and OS-level background runners are still planned work.
