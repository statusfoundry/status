...
- `PluginPackageVerifier` now enforces package SHA-256 matches, Ed25519 signatures against the app-pinned development signing key, and registry revocation checks before a package can be treated as installable. Installed plugin ID, version, and package-hash revocations can be applied locally to mark versions revoked, disable affected plugins, and write audit rows. Production distribution must replace the repository development key with release key custody described in `docs/07-security-privacy.md`; periodic signing-key revocation enforcement still needs stored signing-key ids on installed versions.
