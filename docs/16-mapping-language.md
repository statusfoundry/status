...
`StatusCore` currently includes `MappingConditionEvaluator`, a small evaluator for single mapping conditions and AND groups over normalized resource state. It supports the plain operators and the transition operators (`changed`, `changed_to`, `changed_from`) against current and previous resource snapshots. `StatusCore` also includes the first package mapping runtime: `requests.json`/`mappings.json` decoding, `auth.json` bearer-token, api-key header, basic-auth, and JWT API-key decoding, `setup.schema.json` decoding for canonical `key` fields, JSON selector parsing/resolution, wildcard-tail source iteration, scalar template rendering with the documented scopes, bundled-style shorthand and canonical JSON-form condition parsing, mapped severity tables, request URL/query/header/body rendering, same-host JSON:API/next-link, cursor, and page-number pagination with a page cap and warning output, JSON/probe response normalization, request-payload execution into normalized resources/events/metric points, metric drop event emission against the previous point, warning output for skipped invalid metric points, and persistence of mapped resources/state/events/metrics through the local store and event ingestor. `PluginRuntimeService` connects that runtime to installed plugin packages, saved non-secret account variables, Keychain-backed bearer/api-key/basic/JWT credentials, and app-initiated manual or due cron jobs. Native app shells expose manual runs for configured plugins that declare an enabled manual trigger.
...
Baseline comparison, drop detection, and delta events are core responsibilities, not mapping responsibilities. The mapping only lands points. The current core metric engine emits one warning event per UTC day when a new point drops at least 20% below the previous stored point for the same metric.
