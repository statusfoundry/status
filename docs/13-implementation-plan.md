...
- **WP-8.5 Native registry client and installer UI** — macOS Agent + Plugin Agent. Status: implemented for the v1 native catalog path. The macOS and iOS plugin stores load compatible registry listings from the configured Worker URL, use `PluginInstaller` to download package and manifest artifacts, verify SHA-256/signature/trusted key/revocation state locally, persist installed package versions, and remove installed plugins from local state. Installed plugin rows now surface registry updates when the Worker advertises a newer compatible version, routing updates through the same verified installer path. Remaining future work: richer release notes and permission-diff copy before applying an update.
- **WP-8.6 Registry security review** — Security Agent. Status: implemented for the v1 registry trust boundary. The Worker exposes CORS only for `GET`, `HEAD`, and `OPTIONS`, gives immutable cache headers to package artifacts, serves metadata with short cache lifetimes, filters incompatible versions, and removes revoked plugin IDs, plugin versions, package hashes, and signing key IDs from installable registry responses. Direct requests for revoked versions return `404`. Native package verification remains the final trust boundary and rejects bad hashes, bad signatures, unknown signing keys, revoked plugins/versions/hashes/signing keys, and incompatible package metadata before persistence. Remaining future work: production key-custody and rotation operations for non-development signing keys.
- **WP-8.7 Open-source plugin governance** — Product Agent + Security Agent. Status: implemented for the v1 review-based registry path. `docs/22-plugin-governance.md` defines the `statusfoundry` organization model, the monorepo-first plugin source layout, optional future split repositories for large official plugins, repository-license defaults for official plugins, trust levels (`official`, `verified-third-party`, `local-dev`), the pull-request submission path, maintainer review checklist, signing/publication rules, immutable R2 artifacts, and revocation handling. `CONTRIBUTING.md`, the plugin author guide, and the developer website link to that contract. v1 has no public direct upload to the installable registry; third-party packages require source review before Status signs and publishes them.
